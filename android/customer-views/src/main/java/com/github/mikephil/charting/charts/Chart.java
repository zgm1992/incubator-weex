...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            Log.i("jarvanTest", "bar onLayout   chil is ->"+getChildAt(i)+"  left is ->"+left+" top ->"+ top+" right is ->"+right+" bottom is ->"+bottom);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int measuredWidth = Math.max(getSuggestedMinimumWidth(),
                resolveSize(size,
                        widthMeasureSpec));

        int measuredHeight = Math.max(getSuggestedMinimumHeight(),
                resolveSize(size,
                        heightMeasureSpec));
        ...
        setMeasuredDimension(
                measuredWidth ,
                measuredHeight);
        onSetMeasureDimension(measuredWidth,measuredHeight);
    }
