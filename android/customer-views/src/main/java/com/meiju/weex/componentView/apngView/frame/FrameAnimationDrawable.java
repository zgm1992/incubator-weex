package com.meiju.weex.componentView.apngView.frame;
...
public abstract class FrameAnimationDrawable extends Drawable implements Animatable2Compat, FrameSeqDecoder.RenderListener {
    ...
    public void getFrame(int index, String next) {
        if (next.equals("pause")) {
            pause();
        }
        if (frameSeqDecoder instanceof APNGDecoder) {
            ((APNGDecoder)frameSeqDecoder).getSelectFrame(index, next);
        }
    }

    public void runFrame(int index) {
        frameSeqDecoder.runFrameLimit(index);
    }
